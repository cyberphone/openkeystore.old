...
public class Provisioning
  {
    ...
    public Provisioning (int user_id)
      {
        this.user_id = user_id;
      }


    public Provisioning (int user_id, DebugCallback debug_callback)
      {
        this (user_id);
    ...
    private int createProvisioning (String client_session_id, String server_session_id) throws IOException, SQLException
      {
        int inst = 0;
        Connection conn = KeyUtil.getDatabaseConnection ();
        PreparedStatement pstmt =
             conn.prepareStatement ("INSERT INTO PROVISIONINGS (ClientSession, ServerSession, UserID) VALUES (?, ?, ?)",
    ...
    class DeviceSigner implements SignerInterface
      {
        ...
        private DeviceSigner () throws IOException
          {
            crypto_driver = new CryptoDriver (user_id);
          }


        public X509Certificate[] prepareSigning (boolean fullpath) throws IOException
          {
            cert_path = crypto_driver.getDeviceCertificatePath ();
            return cert_path;
          }
        ...
        public byte[] signData (byte[] data, SignatureAlgorithms algorithm) throws IOException
          {
            return crypto_driver.deviceKeyDigestSign (Digester.digestAll (data, algorithm),
                                                      algorithm);
          }
    ...
    class DeviceDecrypter implements SymmetricKeyDecrypter
      {
        ...
        public byte[] decrypt (byte[] data, X509Certificate optional_key_id) throws IOException
          {
            return new CryptoDriver (user_id).deviceKeyDecrypt (data, AsymEncryptionAlgorithms.RSA_PKCS_1);
          }
      }
  

    class rsaKey implements AsymKeySignerInterface
      {
        KeyPair key_pair;

        rsaKey (KeyPair key_pair)
          {
        ...
        public byte[] signData (byte[] data, SignatureAlgorithms sign_alg) throws IOException, GeneralSecurityException
          {
            ...
            s.initSign (key_pair.getPrivate ());
            s.update (data);
            return s.sign ();
          }
    ...
    class RSAHelper implements AsymKeySignerInterface
      {
        CryptoDriver.KeyPair key_pair;
        RSAHelper (CryptoDriver.KeyPair key_pair)
          {
            this.key_pair = key_pair;
          }
        ...
        public byte[] signData (byte[] data, SignatureAlgorithms algorithm) throws IOException, GeneralSecurityException
          {
            return CryptoDriver.privateKeyDigestSign (Digester.digestAll (data, algorithm),
                                                      key_pair.getPrivateKeyHandle (),
                                                      algorithm);
          }

        public PublicKey getPublicKey () throws IOException, GeneralSecurityException
          {
            return key_pair.getPublicKey ();
          }
      }
    ...
    void createUserKey (KeyOperationRequestDecoder decoder,
                        ...
                        String replace_key_id) throws IOException, SQLException, GeneralSecurityException
      {
        ...
        CryptoDriver.KeyPair key_pair = null;
        X509Certificate archival_key = key_properties.getPrivateKeyArchivalKey ();
        if (false)
          {
            key_pair = new CryptoDriver (user_id).generateKeyPair (key_alg,
                                                                   key_properties.isExportable (),
                                                                   key_properties.getKeyUsage ());

            encoder.addSelfSignedKey (new RSAHelper (key_pair), key_properties.getID ());
          }
        else
          {
            ...
            key_pair = new CryptoDriver (user_id).
            ...
            encoder.addAttestedKey (key_pair.getPublicKey (),
                                    ((CryptoDriver.AttestedKeyPair)key_pair).getAttestSignature (),
                                    key_properties.getID (),
                                    archival_key == null ? null : 
                                      new KeyOperationResponseEncoder.KeyArchivalData
                                         (((CryptoDriver.AttestedKeyPair)key_pair).getEncryptedPrivateKey (),
                                         ((CryptoDriver.AttestedKeyPair)key_pair).getWrappedEncryptionKey (),
                                          encrytion_algorithm,
                                          key_wrap_algorithm));
          }
        ...
        if (key_properties.getPINPolicy () != null)
          {
            ...
            if (key_properties.getPresetPIN () == null)
              {
                pin_blob = KeyUtil.getEncryptedPassphrase (pin_value, pin_policy.getFormat ());
              }
            else
              {
...
*/
    ...
    static class ProvCertSel extends CertificateSupport
      {
        PublicKey public_key;
        javax.security.auth.x500.X500Principal issuer;

        ProvCertSel (int user_id, X509Certificate ca_certificate) throws GeneralSecurityException
          {
            super (user_id);
            ...
            if (ro instanceof KeyOperationRequestDecoder.CreateKey)
            ...
            else
              {
                ...
                if (ro instanceof KeyOperationRequestDecoder.CertificateReference)
                  {
                    Integer[] keys = new ProvCertSel (user_id, ca_cert).getFilteredAndCertified (
                ...
                if (ro instanceof KeyOperationRequestDecoder.DeleteKey)
                ...
                else if (ro instanceof KeyOperationRequestDecoder.DeleteKeysByContent)
                  {
                    ...
                    deleteKeys (new ProvCertSel (user_id, ca_cert).getFilteredAndCertified (
        ...
        for (CredentialDeploymentRequestDecoder.CertifiedPublicKey cred : credep_decoder.getCertifiedPublicKeys ())
          {
            ...
            KeyDescriptor key_descriptor = new KeyMetadataProvider (user_id).getKeyDescriptor (pk.key_id);
...
*/
    ...
    throws IOException, SQLException, GeneralSecurityException
      {
        ...
        byte[] encrypted_symmetric_key = CryptoDriver.provisionPiggybackedSymmetricKey
               (
    ...
    private void updatePresetValue (String get_format_sql, String update_value_sql,
                                    int primary_key_value, String value) throws IOException, SQLException
      {
        ...
        pstmt.setBytes (1, KeyUtil.getEncryptedPassphrase (value, format));
    ...
    public static void storeDeviceCertificatePathAndKey (X509Certificate[] sorted_cert_path,
                                                         PrivateKey private_key,
                                                         int user_id)
    throws IOException, SQLException, GeneralSecurityException
      {
        Provisioning provisioning = new Provisioning (user_id);
        int puk_policy_id = provisioning.createPUKPolicy (1000, PassphraseFormats.NUMERIC);
        provisioning.updatePUKPolicy (puk_policy_id, "01234567890123456789");
        Connection conn = KeyUtil.getDatabaseConnection ();
        ...
        pstmt.setBytes (3, CryptoDriver.sealDevicePrivateKey (private_key.getEncoded ()));
