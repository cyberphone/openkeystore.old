...
public class KeyOperationRequestServlet extends KeyGenServlet
  {    private class KeyCreator      {        KeyOperationRequestEncoder.PUKPolicy puk;

        KeyOperationRequestEncoder.PINPolicy pin;

        ProvisioningState prov_state;        boolean preset;

        KeyCreator (KeyOperationRequestEncoder.PUKPolicy puk, 
                    KeyOperationRequestEncoder.PINPolicy pin,
                    boolean preset,
                    ProvisioningState prov_state)
        ...
        KeyOperationRequestEncoder.KeyProperties create (KeyGen2KeyUsage key_usage, KeyOperationRequestEncoder.KeyAlgorithmData key_alg) throws IOException          {
            KeyOperationRequestEncoder.KeyProperties key_prop = preset ?                prov_state.key_op_req_enc.createKeyWithPresetPIN (key_usage,
                                                                  key_alg,
                                                                  pin,
                                                                  puk,
                                                                  "2478",
        ...
        KeyOperationRequestEncoder.KeyProperties create (KeyGen2KeyUsage key_usage, int rsa_size) throws IOException          {            return create (key_usage, new KeyOperationRequestEncoder.KeyAlgorithmData.RSA (rsa_size));
          }
...
    public void protectedPost (HttpServletRequest request, HttpServletResponse response, HttpSession session) throws IOException, ServletException
      {
        ...
        KeyOperationRequestEncoder encoder =
...
//        key_creator.create (KeyGen2KeyUsage.AUTHENTICATION, 2048);        key_creator.create (KeyGen2KeyUsage.AUTHENTICATION, new KeyOperationRequestEncoder.KeyAlgorithmData.ECC (ECCDomains.P_256));
        ...
        KeyOperationRequestEncoder.ManageObject kmc = encoder.createManageObject ();
