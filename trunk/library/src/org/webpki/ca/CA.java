...
public class CA
  {
    ...
    private void getLogotypes (Vector<BaseASN1Object> entries, Vector<CertSpec.Logotype> raw_list, int index)
      {
        Vector<CertSpec.Logotype> specific_list = new Vector<CertSpec.Logotype> ();
        for (CertSpec.Logotype logotype : raw_list)
          {
            if (logotype.index == index)
              {
                specific_list.add (logotype);
              }
          }
        if (!specific_list.isEmpty ())
          {
            Vector<BaseASN1Object> logotypes = new Vector<BaseASN1Object> ();
            for (CertSpec.Logotype logotype : specific_list)
              {
                Vector<BaseASN1Object> uris = new Vector<BaseASN1Object> ();
                for (String uri : logotype.uris)
                  {
                    uris.add (new ASN1IA5String (uri));
                  }
                logotypes.add (new ASN1Sequence (
                                   new ASN1Sequence (
                                       new ASN1Sequence (
                                           new BaseASN1Object[] {
                                               new ASN1IA5String (logotype.mime_type),
                                               new ASN1Sequence (new ASN1Sequence (
                                                   new BaseASN1Object[]{
                                                       new ASN1Sequence (new ASN1ObjectID (logotype.hash_alg.getOID ())),
                                                       new ASN1OctetString (logotype.hash_value)})),
                                               new ASN1Sequence (uris.toArray (new BaseASN1Object[0]))}))));
              }
            entries.add (new CompositeContextSpecific (index, 
                             new CompositeContextSpecific (0, logotypes.toArray (new BaseASN1Object[0]))));
          }
      }
